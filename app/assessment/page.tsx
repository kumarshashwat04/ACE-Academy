...
export default function AssessmentsPage() {
  ...
  return (
    <AppShell currentTab="assessment">
      ...
      {/* 
        Replaced custom inline layout grids with your cohesive `.a-grid` architecture.
        Grid template columns dynamically balance via the CSS rules.
      */}
      <div className="a-grid" style={{ marginTop: "24px" }}>
        {LEVEL_SCHEMAS.map((level) => {
          ...
          // Build exact dynamic classes mapping your design guidelines
          const cardClasses = [
            "a-card",
            level.cssClass,
            !isUnlocked ? "disabled" : ""
          ...
          return (
            ...
            >
              {/* Icon component mapping standard size rules */}
              <div className="a-ico">
                {isCurrentlyLoading ? "⏳" : level.icon}
              </div>

              {/* Header Title with custom Syne font variables */}
              <div className="a-title">
                {level.name} {isCurrentlyLoading && "..."}
              </div>

              {/* Subtext mapping description blocks */}
              <div className="a-sub">
                {level.subtitle}
              </div>

              {/* Pill containers managing status badges */}
